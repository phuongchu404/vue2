        ...
        <el-row class="el-row-card" :gutter="20">
          <el-card class="box-card">
            ...
            <div style="margin-top: 40px;margin-left: 20px">
              <el-form>
                <el-form-item>
                  <el-button type="primary" @click="downloadTemplate" size="mini"><i class="el-icon-download"/>{{
                      $t('Template')
                    }}
                  </el-button>
                  <el-button type="primary" @click="uploadFile" size="mini"><i class="el-icon-upload2"/>{{
                      $t('Upload')
                    }}
                  </el-button>
                </el-form-item>
            ...
            <div class="from-text">
              ...
              <el-row>
                <el-row class="el-row-card" style="width: 80%;margin-left: 30%;top: 10px;margin: auto">
            ...
            <el-dialog :title="$t('addVisitor')" :visible.sync="ui.uploadDialogVisible" class="addDio">
              <el-form :model="uploadForm" :rules="uploadFormRule" label-width="150px" ref="addForm">

                <el-form-item :label="$t('data-file')" prop="dataFile">
                  <el-input type="file" ref="dataFileRef" v-model="uploadForm.dataFile" accept=".xls, .xlsx"></el-input>
                </el-form-item>
                <el-form-item>
                  <di>
                    <el-progress :text-inside="true" :stroke-width="26" :percentage="uploadPercentage"></el-progress>
                  </di>
                </el-form-item>
              </el-form>
              <div slot="footer" class="dialog-footer">
                <el-button @click.native="handleCancellation()">{{ $t('common.cancel') }}</el-button>
                <el-button type="primary" @click.native="handleUpload()">{{ $t('common.ok') }}</el-button>
              </div>
            </el-dialog>
...
@Component
export default class RegisterNoFace extends Vue {
  info = {
    docNumber: "",
  };
  ui = {
  ...
  uploadFormRule = {
    dataFile: [
      {required: true, message: i18n.t('common.inputFile'), trigger: 'change'},
    ]
  }
  ...
  async handleUpload() {
    let validated = await Utils.validateForm(this.$refs.addForm);
    if (validated) {
      let formData = new FormData();
      let dataFile: any = (this.$refs.dataFileRef as any).$refs.input.files[0];

      formData.append('dataFile', dataFile);
      this.uploadCancel = Axios.CancelToken.source();
      let result = await Utils.doUpload(this, '/api/admin/visitor/upload', formData, this.updateProgress, this.uploadCancel.token);
      if (!result.success) {
        Utils.showWarning(String(i18n.t('common.upload-fail-message')) + " " + i18n.t(result.message));
        this.uploadPercentage = 0;
      } else {
        Utils.showSuccess(String(i18n.t('common.update-success-message')) + " " + result.data + String(i18n.t('record')));
        this.ui.uploadDialogVisible = false;
      }
    }
  }

  updateProgress(percentage: any) {
  ...
  async downloadTemplate() {
    try {
      this.loading = true;
      let resp: any = await Utils.doDownload('GET', `/api/admin/visitor/downloadTemplate`);
      if (resp.success) {
        this.loading = false;
      }
      if (resp && resp.type === 'application/json') {
        let convertStr: any = await Utils.blobToString(resp);
        let convertJSON = JSON.parse(convertStr);
        if (convertJSON.success === false) {
          this.loading = false;
          return;
        }
      }
  ...
  // async downloadTemplate() {
  //   Axios({
  //     url: `/api/admin/visitor/downloadTemplate`, //your url
  //     method: 'GET',
  //     data: null,
  ...
  //     link.href = url;
  //     link.setAttribute('download', "template_.xlsx"); //or any other extension
  ...
  async uploadFile() {
    ...
    this.uploadPercentage = 0;
    this.uploadForm.dataFile = "";
    await Utils.clearValidateForm(this.$refs.addForm);
  }
  ...
  async mounted() {
    ...
    console.log(this.imageDisplay.length)
  };
  ...
  async handleRegistered() {
    ...
    if (result1.success) {
      ...
      if (!result.success) {
        ...
        this.ui.image2 = false;
        this.ui.success = true
        this.message = String(i18n.t('common.insertVisitorSuccess'));
        this.clear();
      }
    }
  }
