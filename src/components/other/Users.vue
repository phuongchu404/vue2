<template>
  <div>
    <div class="box-card search-section">
      ...
      <el-form :model="queryForm" class="form-grid" label-position="left" >
        <el-form-item :label="$t('user.userName')">
          <el-input v-model="queryForm.userName" :placeholder="$t('user.userName')" />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="handleSearch" :disabled="isButtonEnabled('system:user:select')"
                     size="mini">
            <i class="el-icon-search"/><span>{{ $t('option.query') }}</span>
          </el-button>
    ...
    <div class="page-table" style="padding-bottom: 40px">
      <el-row :gutter="20">
        <el-table :data="filteredTableData" style="width: 97%;margin-top: 10px;margin-left: 20px;"
          ...
          <el-table-column prop="realName" :label="$t('user.realName')" min-width="100"></el-table-column>
    ...
    <el-dialog :title="ui.addRecord?$t('user.add'):$t('user.update')" :visible.sync="ui.dialogVisible" width="40%"
               class="addDio">
      <el-form :model="editForm" :rules="editFormRules" label-width="150px" label-position="left" ref="editForm">
        <input type="hidden" v-model="editForm.id"></input>
        <el-form-item :label="$t('user.userName')" prop="userName">
          <el-input v-model="editForm.userName" autofocus :readonly="!ui.addRecord" ></el-input>
        </el-form-item>
...
export default class Users extends Vue {
  ...
  editFormRules = {
    ...
    phoneNumber: [
      {required: true, message: i18n.t("user.inputPhoneNumber"), trigger: 'blur'},
      { pattern: /^[0-9]{9,15}$/, message: i18n.t("user.phoneNumberFormat"), trigger: 'blur' }
    ],
    mail: [
      {required: true, message: i18n.t("user.inputEmail"), trigger: 'blur'},
      { type: 'email', message: i18n.t("user.emailFormat"), trigger: ['blur', 'change'] }
    ],
  };
  passwordRules = {
  ...
  async loadTableData() {
    let param = `userName=${this.queryForm.userName}&pageNo=${this.pageNumber}&pageSize=${this.pageSize}`
    let result = await Utils.doGet(this, `/api/admin/users?${param}`);
    if (result.success) {
  ...
  async loadAllRoles() {
    let result = await Utils.doGet(this, '/api/admin/roles/list');
    if (result.success) {
